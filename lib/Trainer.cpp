...
Trainer::Trainer(const Save *sav) {
  ...
  trainer = PokeText::toPoketext(
      (const uint16_t *)&sav->BlockA[tOffset[offsetName][type]], 8);
  rival = PokeText::toPoketext(
      (const uint16_t *)&sav->BlockA[tOffset[offsetRival][type]], 8);
...
void Trainer::saveInto(Save *sav) {
  ...
  // Set Items
  for (int i = 0; i < 8; ++i) {
    memcpy(&sav->BlockA[itemOffset[ntype][i]], items[i],
           min(itemNum[type][i], itemNum[ntype][i]));
    //		for(int j = 0; j < itemNum[type][i]; ++j) {
    //			items[i][j] = 0; //Get item from save
    //		}
  }
