...
class DLL_EXPORT Pokemon {
 public:
  ...
  bool isShiny() const;
  // Species specific
  // Set unown form support, use a char 'A' etc to specify. Support '!' and '?'
  // aswell.
  void setUnownForm(char f);
  // Some basic utility functions! These are called automatically by Box and
  // Party classes. These deshuffle+decrypt, and shuffle+encrypt respectively
  void decrypt();
  void encrypt();
};
